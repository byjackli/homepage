import React from 'react';
import { Card, Slot } from '../components/Nodes';
import { Intext } from '../components/Clicks';

function Resume() {

    return (
        <main className="resume">
            <div className="hrzBT">
                <Card className="card" header="Contact" elements={
                    <div>
                        <div className="hrzTL">
                            <ol>
                                <li><i class="fas fa-phone"></i> (347) 476 - 4944</li>
                                <li><i class="fas fa-envelope"></i> jackli@buffalo.edu</li>
                                <li><i class="fas fa-envelope"></i> mail@byjackli.com</li>
                            </ol>
                            <ol>
                                <li><i class="fab fa-linkedin"></i> <Intext href="https://www.linkedin.com/in/byjackli/" label="LinkedIn" /></li>
                                <li><i class="fab fa-github"></i> <Intext href="https://github.com/byjackli/" label="Github" /></li>
                                <li><i class="fab fa-twitter"></i> <Intext href="https://twitter.com/byjackli" label="Twitter" /></li>
                            </ol>
                        </div>
                    </div>
                } />
                <Card className="card" header="Social Media" elements={
                    <div>
                        <p>Feel free to reach out and connect with me, socially!</p>
                        <div className="hrzTL">
                            <ol>
                                <li><i class="fab fa-instagram"></i> <Intext href="https://www.instagram.com/byjackli/" label="Instagram" /></li>
                                <li><i class="fab fa-pinterest"></i> <Intext href="https://www.pinterest.com/byjackli/" label="Pinterest" /></li>
                            </ol>
                            <ol>
                                <li><i class="fab fa-spotify"></i> <Intext href="https://open.spotify.com/user/primeno17?si=qymzYYxATCmN-SRbfeJdnA" label="Spotify" /></li>
                                <li><i class="fab fa-facebook"></i> <Intext href="https://www.facebook.com/byjackli/" label="Facebook" /></li>
                            </ol>
                            <ol>
                                <li><i class="fab fa-youtube"></i> <Intext href="https://www.youtube.com/wizardjack/" label="YouTube" /></li>
                            </ol>
                        </div>
                    </div>
                } />
                <Card className="card" header="Location" elements={
                    <div>
                        <div className="hrzTL">
                            <ol>
                                <li><i class="fas fa-map-marker-alt"></i> Currently home. New York, NY</li>
                                <li><i class="fas fa-home"></i> New York, NY</li>
                                <li><i class="fas fa-graduation-cap"></i> Buffalo, NY</li>
                            </ol>
                        </div>
                    </div>
                } />
            </div>

            <div className="hrzBT">
                <Card className="card" header="All Current Agendas" elements={
                    <div>
                        <p>On-going projects, tasks, and more.</p>
                        <p>Updated every Sunday morning, 5am EDT.</p>
                        <div className="card-cap">
                            <Slot href="#" month="September" date="22" header="background image" image="https://scontent-lga3-1.cdninstagram.com/v/t51.2885-15/e35/70515020_421309365191643_6585258917258799635_n.jpg?_nc_ht=scontent-lga3-1.cdninstagram.com&_nc_cat=103&_nc_ohc=Zolkr2asqEEAX-A_w3N&oh=26584734bf49b0404bc4115dd3a9d89e&oe=5EF10579" />
                            <Slot href="#" month="May" date="4" header="no background image" />
                            <Slot href="#" month="July" date="11" header="background image" image="https://scontent-lga3-1.cdninstagram.com/v/t51.2885-15/e35/62054280_837856996600363_8628047517587627838_n.jpg?_nc_ht=scontent-lga3-1.cdninstagram.com&_nc_cat=107&_nc_ohc=uOIFhXKuPo4AX-o6lwg&oh=c0577da8f21509d64e81733abdf82b0c&oe=5EF183EB" tags={["boat", "car", "train"]} />
                        </div>
                    </div>
                } />
                <Card className="card" header="Education" elements={
                    <div>
                        <p>University at Buffalo - Expected May 2021.</p>
                        <p>Computer Science Major, Communications Minor</p>
                        <div className="card-cap">
                            <Slot href="#" month="September" date="22" header="background image" image="https://scontent-lga3-1.cdninstagram.com/v/t51.2885-15/e35/70515020_421309365191643_6585258917258799635_n.jpg?_nc_ht=scontent-lga3-1.cdninstagram.com&_nc_cat=103&_nc_ohc=Zolkr2asqEEAX-A_w3N&oh=26584734bf49b0404bc4115dd3a9d89e&oe=5EF10579" />
                            <Slot href="#" month="May" date="4" header="no background image" />
                            <Slot href="#" month="July" date="11" header="background image" image="https://scontent-lga3-1.cdninstagram.com/v/t51.2885-15/e35/62054280_837856996600363_8628047517587627838_n.jpg?_nc_ht=scontent-lga3-1.cdninstagram.com&_nc_cat=107&_nc_ohc=uOIFhXKuPo4AX-o6lwg&oh=c0577da8f21509d64e81733abdf82b0c&oe=5EF183EB" tags={["boat", "car", "train"]} />
                        </div>
                    </div>
                } />
                <Card className="card" header="Projects" elements={
                    <div>
                        <p>Only past computer science projects and academic assignments listed in this section.</p>
                        <div className="card-cap">
                            <Slot href="#" month="September" date="22" header="background image" image="https://scontent-lga3-1.cdninstagram.com/v/t51.2885-15/e35/70515020_421309365191643_6585258917258799635_n.jpg?_nc_ht=scontent-lga3-1.cdninstagram.com&_nc_cat=103&_nc_ohc=Zolkr2asqEEAX-A_w3N&oh=26584734bf49b0404bc4115dd3a9d89e&oe=5EF10579" />
                            <Slot href="#" month="May" date="4" header="no background image" />
                            <Slot href="#" month="July" date="11" header="background image" image="https://scontent-lga3-1.cdninstagram.com/v/t51.2885-15/e35/62054280_837856996600363_8628047517587627838_n.jpg?_nc_ht=scontent-lga3-1.cdninstagram.com&_nc_cat=107&_nc_ohc=uOIFhXKuPo4AX-o6lwg&oh=c0577da8f21509d64e81733abdf82b0c&oe=5EF183EB" tags={["boat", "car", "train"]} />
                        </div>
                    </div>
                } />
            </div>

            <div className="hrzBT">
                <Card className="card" header="Expertises" elements={
                    <div>
                        <p>temporary text</p>
                    </div>
                }
                    style={{ width: "100%" }}
                />
            </div>
        </main>
    );
}

export default Resume;